...
export default function AdminCrmSettings() {
  ...
  const handleSave = async () => {
    setToast(null);
    if (!url.trim()) {
      setToast({ message: "Please enter a CRM form URL", type: "error" });
      return;
    }
    try {
      new URL(url.trim());
    ...
    try {
      const res = await fetch("/api/admin/crm-settings", {
        ...
        body: JSON.stringify({ contactFormUrl: url.trim() }),
      });
      const data = await res.json();
      if (data.success) {
        setSavedUrl(data.contactFormUrl);
        setToast({ message: "CRM URL saved successfully", type: "success" });
        setPreviewKey((k) => k + 1);
      } else {
  ...
  const handleRemove = async () => {
    ...
    setRemoving(true);
    try {
      const res = await fetch("/api/admin/crm-settings", { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
        setUrl("");
        setSavedUrl("");
        setToast({ message: "CRM URL removed. Contact page will show the default form.", type: "success" });
        setPreviewKey((k) => k + 1);
      } else {
    ...
    <div className={`adm-page ${mounted ? "adm-page-in" : ""}`}>
      ...
      <div className="adm-header">
        <div className="adm-header-text">
          <h1 className="adm-title">CRM Integration</h1>
          <p className="adm-sub">Connect an external CRM contact form to your website</p>
        </div>
      </div>
      ...
      <div className="adm-settings-grid">
        ...
        <div className="adm-settings-card">
          <div className="adm-settings-card-header">
            ...
            <div>
              <h3 className="adm-settings-card-title">CRM Contact Form URL</h3>
              <p className="adm-settings-card-sub">Paste the URL of your CRM embed form</p>
            </div>
          </div>
            ...
            <>
              <div className="adm-settings-field">
                ...
                <input
                  ...
                  placeholder="https://your-crm.com/form/embed-url"
                />
              </div>
              ...
              {savedUrl && (
                <div style={{ marginTop: "18px", padding: "12px 16px", background: "rgba(46,107,78,.06)", borderRadius: "10px", fontSize: "13px", color: "#2E6B4E" }}>
                  Active CRM form is embedded on the <a href="/contact" target="_blank" rel="noopener noreferrer" style={{ color: "#2E6B4E", fontWeight: 600 }}>Contact page</a>.
                </div>
              )}
            </>
          )}
        ...
        <div className="adm-settings-card">
          <div className="adm-settings-card-header">
            ...
            <div>
              <h3 className="adm-settings-card-title">CRM Form Preview</h3>
              <p className="adm-settings-card-sub">See how the embedded form will appear</p>
            </div>
          </div>

          {previewUrl ? (
            <CrmPreview key={previewKey} url={previewUrl} />
          ) : (
            <div style={{ padding: "40px 20px", textAlign: "center", color: "#54607A", fontSize: "14px", border: "1px dashed #E6DFD3", borderRadius: "12px" }}>
              ...
              <p style={{ margin: 0 }}>Enter a CRM URL and save to see the preview</p>
            </div>
          )}
        </div>
      </div>
  ...
  if (failed) {
    return (
      <div style={{ padding: "40px 20px", textAlign: "center", color: "#8A2434", fontSize: "14px", border: "1px dashed #E6DFD3", borderRadius: "12px" }}>
        ...
        <p style={{ margin: "0 0 14px" }}>Unable to load CRM Contact Form.</p>
        <button
